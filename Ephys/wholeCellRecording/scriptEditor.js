my_widget_script =
{
    ...
    test_data: function () {
        ...
        var output = { widgetData: testData, seriesNums: [1, 4, 5] };
    ...
    initDynamicContent: function (parsedJson) {
        if(parsedJson.seriesNums){
            this.seriesNums = parsedJson.seriesNums;
            for(var i=0; i<parsedJson.seriesNums.length; i++){
                var seriesNum = parsedJson.seriesNums[i];
                this.makeSeriesCard(seriesNum);
            }
        }
    },
    ...
    setUpInitialState: function () {
       ...
       $(".copyData").on("click", (e)=> {
        ...
        $("#addSeries").on("click", (e)=>{
            this.addSeries();
        });

        $(".stimType").each((i,e)=>{
            this.showHideOptoProps($(e));
        })
    ...
    **/
    ...
    addSeries: function(){
        if(this.seriesNums.length > 0){
            var lastSeries = this.seriesNums[this.seriesNums.length - 1];
            var seriesNum = lastSeries + 1;
        } else {
            var seriesNum = 1;
        }

        this.seriesNums.push(seriesNum);

        this.makeSeriesCard(seriesNum);
    },

    makeSeriesCard: function (seriesNum){
        var $div = $("#seriesCardDiv");
        ...
        var $seriesDiv = $(".cardDiv" + seriesSearch);

        var header = $("<h3></h3>").append("Series <span data-series='"+seriesNum+"' data-calc='seriesNum'>"+seriesNum+"</span>");

        var $body = this.makeSeriesCardBody(seriesNum);

        this.makeCard($seriesDiv, header, $body);
    },

    makeSeriesCardBody: function(seriesNum){
        $body = $("<div></div>");
        ...
        var initialRows = [
            ...
            },
            {
                ...
                label: "Passive Series Before",
                type: "number",
                className: "passBefore"
            }, {
                label: "Passive Series After",
                type: "number",
                className: "passAfter"
            },{
                label: "Spontaneous",
                ...
                label: "1Hz, 1ms, 0V:",
                type: "button",
                className: "fill1Hz1ms0V",
                addRowClass: " hideView",
            },{
                label: "20Hz, 1ms, 0V:",
                type: "button",
                className: "fill20Hz1ms0V",
                addRowClass: " hideView",
            },{
                label: "Duration (sec):",
                type: "number",
        ...
        for(row of initialRows){
            $body.append(
                this.makeRowFromObj(row, seriesNum)
            )
        }

        $body.find(".deleteSeries").addClass("disableOnView").prop("value", "Delete Series").on("click", (e)=>{
            this.deleteSeries(seriesNum);
        });

        $body.find(".fill1Hz1ms0V").addClass("disableOnView").prop("value", "1Hz, 1ms, 0V").on("click", (e)=>{
            this.updateTypeSettings(seriesNum, "opto", 1, 1, 0);
        });

        $body.find(".fill20Hz1ms0V").addClass("disableOnView").prop("value", "20Hz, 1ms, 0V").on("click", (e)=>{
            this.updateTypeSettings(seriesNum, "opto", 20, 1, 0);
        });

        $body.find(".fillSpont").addClass("disableOnView").prop("value", "spontaneous").on("click", (e)=>{
            this.updateTypeSettings(seriesNum, "spontaneous", null, null, null);
        });

        $body.find(".stimType").on("change", (e)=>{
            var $el = $(e.currentTarget);
            this.showHideOptoProps($el);
        }).each((i,e)=>{
            var $el = $(e);
            this.showHideOptoProps($el);
        })
    ...
    updateTypeSettings: function(seriesNum, type, freq, pulseDur, stimVolt){
        var seriesSearch = this.seriesSearch(seriesNum);

        $(".stimType"+seriesSearch).val(type);
        this.showHideOptoProps($(".stimType"+seriesSearch));
        $(".stimFreq"+seriesSearch).val(freq);
        $(".pulseDur"+seriesSearch).val(pulseDur);
        $(".stimVolt"+seriesSearch).val(stimVolt);
    },

    showHideOptoProps: function($stimType){
        var stimType = $stimType.val();
        var $parentRow = $stimType.closest(".row");

        $parentRow.siblings(".ifOpto").hide();
        if(stimType==="opto"){
            $parentRow.siblings(".ifOpto").show()
        }
    },

    deleteSeries: function(seriesNum){
        // Remove it from seriesNums
        var index = this.seriesNums.indexOf(seriesNum);
        if(index > -1){
            this.seriesNums.splice(index, 1);
        }
    ...
    makeInput: function(inputType, className, seriesNum, optionsObj){
        var lowerCaseName = className.toLowerCase();
        if(inputType === "select"){
            $input = $("<select></select>", {
                "name": lowerCaseName+seriesNum,
                "id": className+seriesNum,
    ...
    makeRowFromObj: function(obj, seriesNum){
