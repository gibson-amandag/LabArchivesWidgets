...
widgetData:testData,seriesNums:[1,4,5]}
;return JSON.stringify(output)},
...
this.showOther($(e))}).on("click",e=>{
this.showOther($(e.currentTarget))
}),$("#addSeries").on("click",e=>{this.addSeries()
}),$(".stimType").each((i,e)=>{
this.showHideOptoProps($(e))}),this.resize()},
seriesNums:[],resize:function(){
...
var watch=$el.data("watch"),calcSearch=my_widget_script.calcSearch(watch),dayNum=$el.data("day"),mouseNum=$el.data("mouse"),seriesNum=$el.data("series"),val=$el.val()
;if(dayNum)calcSearch+=my_widget_script.daySearch(dayNum)
;if(mouseNum)calcSearch+=my_widget_script.mouseSearch(mouseNum)
;if(seriesNum)calcSearch+=my_widget_script.seriesSearch(seriesNum)
;if("mouse"==watch)if(!val)val="Mouse "+mouseNum
;if("seriesNum"==watch)if(!val)val=seriesNum
...
addSeries:function(){
if(this.seriesNums.length>0)var lastSeries=this.seriesNums[this.seriesNums.length-1],seriesNum=lastSeries+1;else var seriesNum=1
...
;var seriesSearch=this.seriesSearch(seriesNum),$seriesDiv=$(".cardDiv"+seriesSearch),header=$("<h3></h3>").append("Series <span data-series='"+seriesNum+"' data-calc='seriesNum'>"+seriesNum+"</span>"),$body=this.makeSeriesCardBody(seriesNum)
;this.makeCard($seriesDiv,header,$body)},
makeSeriesCardBody:function(seriesNum){
$body=$("<div></div>");var initialRows=[{
label:"Delete Series:",type:"button",
className:"deleteSeries",addRowClass:" hideView"
},{label:"Series #",type:"number",
className:"seriesNum"},{
label:"Passive Series Before",type:"number",
className:"passBefore"},{
label:"Passive Series After",type:"number",
className:"passAfter"},{label:"Spontaneous",
type:"button",className:"fillSpont",
addRowClass:" hideView"},{label:"1Hz, 1ms, 0V:",
type:"button",className:"fill1Hz1ms0V",
addRowClass:" hideView"},{label:"20Hz, 1ms, 0V:",
type:"button",className:"fill20Hz1ms0V",
addRowClass:" hideView"},{label:"Duration (sec):",
type:"number",className:"seriesDur"},{
label:"Series type:",type:"select",
className:"seriesType",optionsObj:[{value:"",
text:"[Select]"},{value:"currentClamp",
text:"current-clamp"},{value:"voltageClamp",
text:"voltage-clamp"}]},{label:"Stimulation:",
type:"select",className:"stimType",optionsObj:[{
value:"",text:"[Select]"},{value:"spontaneous",
text:"spontaneous"},{value:"opto",text:"opto"},{
value:"other",text:"other"}]},{label:"Frequency:",
type:"number",className:"stimFreq",
addRowClass:"ifOpto"},{label:"Pulse duration:",
type:"number",className:"pulseDur",
addRowClass:"ifOpto"},{label:"Stim voltage:",
type:"number",className:"stimVolt",
addRowClass:"ifOpto"},{label:"Notes:",
type:"textarea",className:"notes"}]
;for(row of initialRows)$body.append(this.makeRowFromObj(row,seriesNum))
;return $body.find(".deleteSeries").addClass("disableOnView").prop("value","Delete Series").on("click",e=>{
...
;this.showHideOptoProps($el)}).each((i,e)=>{
var $el=$(e);this.showHideOptoProps($el)}),$body},
...
var stimType=$stimType.val(),$parentRow=$stimType.closest(".row")
;if($parentRow.siblings(".ifOpto").hide(),
"opto"===stimType)$parentRow.siblings(".ifOpto").show()
},deleteSeries:function(seriesNum){
var index=this.seriesNums.indexOf(seriesNum)
;if(index>-1)this.seriesNums.splice(index,1)
;var seriesSearch=this.seriesSearch(seriesNum)
...
"data-watch":className,"data-series":seriesNum
...
type:inputType,name:lowerCaseName+seriesNum,
id:className+seriesNum,
class:className+" fullWidth watch",
"data-watch":className,"data-series":seriesNum})
...
var $row=this.makeRow(obj.label,this.makeInput(obj.type,obj.className,seriesNum,obj.optionsObj),obj.addRowClass)
;return $row},updateTextarea:function(textarea){
