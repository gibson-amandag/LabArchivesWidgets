...
images:{1:{}}};return JSON.stringify(output)},
...
;this.updateFileName(imageNum)}}),this.resize()},
resize:function(){
...
var $div=$("#imgDiv"),$header="<span data-calc='imgNum' data-image='"+imgNum+"'>Image "+("000"+imgNum).slice(-3)+"</span>",myLeftCol="col-12 col-sm-6 col-md-4 col-lg-3 text-left text-sm-right",$body=$("<div></div>",{
class:"container"
...
type:"number",step:1,min:0,value:imgNum
...
}).after("<hr/>"))))).append(this.makeRow("Observations",$("<text"+"area></tex"+"tarea>",{
...
e.target.style.height="auto",e.target.style.height=e.target.scrollHeight+"px",
this.resize()})))
;this.makeCard($div,$header,$body),$div.find(".card").last().attr("data-image",imgNum).data("image",imgNum),
...
var imageSearch=this.imageSearch(imgNum),$div=$(".opticalConfigDiv"+imageSearch),channelSearch=this.channelSearch(configNum),fluorTarget=$(".fluor"+channelSearch).val(),defIntensity=$(".defIntensity"+channelSearch).val(),defExposure=$(".defExposure"+channelSearch).val(),printConfig=parseInt(configNum)+1
;$div.append($("<div></div>",{
...
;if(autoUpdate){
var realNumber=("000"+$(".imgNum"+imageSearch).val()).slice(-3),date=$(".captureDate"+imageSearch).val(),objective=$(".objective"+imageSearch).val(),region=$(".region"+imageSearch).val(),mouseID=$("#mouseID").val(),seriesID=$("#seriesID").val(),sectionID=$("#sectionID").val(),nameString=""
;if(mouseID){
