...
;var numWashes=5
;if(parsedJson.numWashes)numWashes=parsedJson.numWashes
...
abNums:this.abNums,numWashes:this.numWashes}
...
;this.addAntibodies(abNum)}var numWashes=5
;if(parsedJson.numWashes)numWashes=parsedJson.numWashes
...
var solns=this.blockingSolnCalc()
...
}),$("#numWashes").each((i,e)=>{}).on("change",e=>{
this.updateWashTracking($(e.currentTarget).val())
}),$("#washDur").on("change",e=>{
var timeText=this.getTimeText($(e.currentTarget).val())
;$(".updateWashDur").attr("data-time",timeText),
$(".updateWashDur").data("time",timeText),
$(".watchTime").each((i,e)=>{
var $elToWatch=$(e),$elToFill=$(e).parent().next().find($(".fillTime"))
;this.watchTime($elToWatch,$elToFill)})
}),$("#blockDur").on("change",e=>{
var blockDur=$(e.currentTarget).val(),timeText=this.getTimeText(blockDur)
;$(".updateBlockDur").attr("data-time",timeText),
$(".updateBlockDur").data("time",timeText),
$(".watchTime").each((i,e)=>{
var $elToWatch=$(e),$elToFill=$(e).parent().next().find($(".fillTime"))
;this.watchTime($elToWatch,$elToFill)})
}),this.blockingSolnCalc(),this.PBST1Calc(),
this.PBST2Calc(),this.primaryAbSolnCalc(),
...
var desiredFinalVol=$("#desiredPBSTBlockVol").val(),percNGS=$("#percNGS").val(),percTriton=$("#percTriton").val()
;if(!(percNGS>0)||percNGS>100)percNGS=4,
$("#percNGS").val(percNGS),this.watchValue($("#percNGS"))
;if(!(percTriton>0)||percTriton>100)percTriton=.4,
$("#percTriton").val(.4),this.watchValue($("#percTriton"))
;var dilFractionNGS=percNGS/100,dilFractionTriton=percTriton/100
;if(desiredFinalVol>0)var startVolNGSmL=this.startVolOfDilutionCalc(dilFractionNGS,desiredFinalVol),startVolNGSuL=+(1e3*startVolNGSmL).toFixed(2),startVolTritonx100=this.startVolOfDilutionCalc(dilFractionTriton,desiredFinalVol),startVolTriton20percmL=5*startVolTritonx100,startVolTriton20percuL=+(1e3*startVolTriton20percmL).toFixed(2),startVolPBS=+(desiredFinalVol-startVolNGSmL-startVolTriton20percmL).toFixed(2);else var startVolNGSuL="[Enter desired amount]",startVolTriton20percuL="[Enter desired amount]",startVolPBS="[Enter desired amount]"
;return solns={NGS:startVolNGSuL,
triton:startVolTriton20percuL,PBS:startVolPBS
},$("#volNGSBlock").text(solns.NGS),
$("#volTritonBlock").text(solns.triton),$("#volPBSBlock").text(solns.PBS),
solns},PBST1Calc:function(version){
...
solns},primaryAbSolnCalc:function(){
var $primaryVol=$("#desiredPrimaryVol"),primaryVol=$primaryVol.val()
;if(primaryVol>0)$(".abDilution").each((i,e)=>{
var abNum=$(e).data("abnum"),abSearch=this.abNumSearch(abNum),dilutionFactor=$(e).val()
;if(dilutionFactor>0){
var dilutionVal=1/dilutionFactor,amountAb_mL=this.startVolOfDilutionCalc(dilutionVal,primaryVol),amountAb_uL=+(1e3*amountAb_mL).toFixed(2),stockDilution=$(".abStock"+abSearch).val()
...
});else primaryVol="[Enter desired volume]",
$(".abDilution").each((i,e)=>{
...
$(".abDilution"+abSearch).val("2000"),abNum++
;var inArray=this.checkInArray(abNum,this.abNums)
;if(!inArray)this.addAntibodies(abNum)
;var abSearch=this.abNumSearch(abNum)
...
for(i=currentNumWashes;i<numWashes;i++)this.makeWashRow(i)
...
;$(washSearch).remove()}this.numWashes=numWashes
}else $("#numWashes").val(currentNumWashes)
...
}).append("Wash "+printNum+" complete at <span class='fillTime purple updateWashDur' data-time='"+timeText+"'>{Enter Start Time}</span>")))
}};