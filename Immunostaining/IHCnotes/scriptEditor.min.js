...
$(".regionNotes").text(""),$(".regDiv").hide(),$(".updateThickness").on("click",e=>{
this.runIfConfirmed("Are you sure you want to update all?",()=>{
$(".sectionSize").val($("#defThickness").val())})
}),this.resize()},regDivOn:false,
...
className:"DOB",addRowClass:"updateMouseObj"},{
label:"Age in Days:",type:"number",
className:"ageInDays",addRowClass:"updateMouseObj"
...
mouseLabels:["mouseID","cycleID","trt","time","DOB","ageInDays","sacDate","transportTime","sacTime","uterineDescription","salineTime","perfusionTime","liverCleared","formalinMovement","extractionTime","sucroseTime","sectionDate","sectionSize"],
...
$("#minAgeFilter").val("70"),$("#maxAgeFilter").val("180")
...
;if(this.matchesName(info.mouseID,mouseFilter)&&this.matchesFilter(trtGroup,trtGroupFilter)&&this.matchesDate(info.sectionDate,sectionedFilter)&&(!info.ageInDays||parseFloat(info.ageInDays)<=maxAgeFilter&&parseFloat(info.ageInDays)>=minAgeFilter))keepMouse=true
;return keepMouse},
...
debugger
;var thisID=obj.mouseID,mouseNum,updatingMouse=false
;if(thisID){
var matchInfo=this.getMatchingObjSubset(this.mice,"mouseID",thisID)
;if(matchInfo.matchingObj.length>0)mouseNum=matchInfo.matchingObj[0],
updatingMouse=true;else mouseNum=this.getNextNum(this.mouseNums),
this.addMouse(mouseNum)
}else mouseNum=this.getNextNum(this.mouseNums),this.addMouse(mouseNum)
;var mouseSearch=this.mouseSearch(mouseNum),headers=this.mouseLabels
;for(header of headers){
var thisVal=obj[header],updateVal=true
;if(updatingMouse&&!this.checkInArray(header,uploadHeaders))updateVal=false
;if(updateVal)$("."+header+mouseSearch).val(obj[header]),
