my_widget_script={damGenerationNums:[],damGenerations:{},sireGenerationNums:[],sireGenerations:{},
damNums:[],dams:{},sireNums:[],sires:{},mode:"edit",init:function(mode,json_data){
...
onload()}else onload()},head.appendChild(script)},myInit:function(mode,json_data){this.mode=mode
...
var testData=JSON.parse(this.parent_class.test_data()),output={widgetData:testData}
;return JSON.stringify(output)},is_valid:function(b_suppress_message){
...
for(genNum of parsedJson.sireGenerationNums)this.makeSireGenCard(genNum)
...
for(sireNum of parsedJson.sireNums)this.makeSireCard(sireNum);this.sireNums=parsedJson.sireNums}
...
;var breedingNums=this.dams[damNum].breedingNums;if(breedingNums)for(breedingNum of breedingNums){
...
$(".editOnView").find("option").prop("disabled",""),$(".hideView").hide(),$("#dueDate").datepicker({
dateFormat:"yy-mm-dd"}),$("#careDate").datepicker({dateFormat:"yy-mm-dd"
...
;input.setAttribute("type","date");var supported=true;if("date"!==input.type)supported=false
;return this.dateSupported=supported,input.remove(),supported},dateSupported:true,
checkDateFormat:function($dateInput){if(!this.dateSupported){
...
e.setAttribute("style","height:"+e.scrollHeight+"px;overflow-y:hidden;")}).on("input",e=>{
e.target.style.height="auto",e.target.style.height=e.target.scrollHeight+"px",this.resize()}),
...
this.getDamsDue($(e).val())}),$(".htmlCardHeader").on("click",e=>{
this.toggleCard($(e.currentTarget))}),$("#convertToTable").on("click",e=>{
...
;if($el.is(":checked"))checkState=true;return checkState},resize:function(){
this.parent_class.resize_container()},checkForNames:function(){
...
getDynamicContent:function(){var dynamicContent={};return dynamicContent},
...
;if(text)thisMessage=text;var top="auto";if(elForHeight)top=elForHeight.offsetTop+"px"
;bootbox.confirm({message:thisMessage,callback:proceed=>{if(proceed)functionToCall()}}),
$(".modal-dialog").css("top",top)},dialogConfirm:function(text,functionToCall,elForHeight=null){
var thisMessage="Do you want to proceed?";if(text)thisMessage=text;var top="auto"
;if(elForHeight)top=elForHeight.offsetTop+"px";bootbox.confirm({message:thisMessage,
...
;if(prompt)thisTitle=prompt;var top="auto";if(elForHeight)top=elForHeight.offsetTop+"px"
;bootbox.prompt({title:thisTitle,callback:result=>{functionToCall(result)}
}),$(".modal-dialog").css("top",top)},checkInArray:function(searchVal,array){
var proceed=-1!==$.inArray(searchVal,array);return proceed},dataSearch:function(dataName,dataValue){
var dataSearch="[data-"+dataName+"='"+dataValue+"']";return dataSearch},tableSearch:function(table){
var str=this.dataSearch("table",table);return str},calcSearch:function(calc){
var str=this.dataSearch("calc",calc);return str},daySearch:function(day){
var str=this.dataSearch("day",day);return str},mouseSearch:function(mouse){
var str=this.dataSearch("mouse",mouse);return str},damSearch:function(dam){
var str=this.dataSearch("dam",dam);return str},sireSearch:function(sire){
...
toggleTableFuncs:function($table,$errorMsg){this.data_valid_form($errorMsg),$table.toggle(),
this.resize()},toCSVFuncs:function(fileName,tableID,$errorMsg){
...
dLink.download=filename,dLink.href=window.URL.createObjectURL(csvFile),dLink.style.display="none",
document.body.appendChild(dLink),dLink.click()},exportTableToCSV:function(filename,table){
...
makeCard:function($div,cardHeadContent,cardBodyContent){$div.append($("<div></div>",{class:"card"
}).append($("<button></button>",{type:"button",class:"card-header"}).on("click",e=>{
this.toggleCard($(e.currentTarget))}).append(cardHeadContent)).append($("<div></div>",{
class:"card-body collapse"}).append(cardBodyContent))),this.resize()},addDam:function(damNum){
var inArray=this.checkInArray(damNum,this.damNums);if(!inArray)this.damNums.push(damNum),
this.dams[damNum]={damID:"",damGeneration:"",breedingNums:[],breedings:{}},this.makeDamCard(damNum)
...
;var $damDiv=$(".damCard"+this.damSearch(damNum)),header=$("<div></div>",{class:"damIDCalc",
"data-calc":"damID","data-dam":damNum}).append("Dam "+damNum),$body=this.makeDamCardBody(damNum)
;this.makeCard($damDiv,header,$body),
this.makeDamGenList(damNum),$("#damSelect").append($("<option></option>",{value:damNum,
...
className:"deleteDam",addRowClass:"hideView"},{label:"Generation:",type:"select",
className:"damGeneration",addRowClass:"updateDamObj",optionsObj:[{value:"",text:"[Select]"}]},{
label:"Add breeding:",type:"button",className:"addBreeding",addRowClass:"hideView"}]
...
}),$body.find(".updateDamObj").on("change",e=>{var $el=$(e.target)
;this.updateObjFromVal($el,this.dams[damNum])}),$body},updateObjFromVal:function($el,obj){
var val=$el.val();valSafe=this.encodeHTML(val);var thisProp=$el.data("watch")
;if(thisProp)obj[thisProp]=valSafe},addSire:function(sireNum){
var inArray=this.checkInArray(sireNum,this.sireNums);if(!inArray)this.sireNums.push(sireNum),
this.sires[sireNum]={sireID:"",sireGeneration:""},this.makeSireCard(sireNum)},
makeSireCard:function(sireNum){var $div=$("#sireCardDiv")
;if(!$div.find(".card").length)$div.html("");var row="row mt-2",col="col-12 col-lg-6"
;$div.append($("<div/>",{class:"col col-md-6 mt-2 sireCard","data-sire":sireNum}))
;var $sireDiv=$(".sireCard"+this.sireSearch(sireNum)),header=$("<div></div>",{class:"sireIDCalc",
...
this.makeSireGenList(sireNum),$("#sireSelect").append($("<option></option>",{value:sireNum,
class:"sireIDCalc","data-calc":"sireID","data-sire":sireNum
}).append("Sire "+sireNum)).attr("size",Math.min(this.sireNums.length,15)),
this.updateSireList(sireNum)},makeSireList:function(damNum,breedingNum){
...
;$("select.sireBreeding"+damBreedingSearch).append($("<option></option>",{value:sireNum,
"data-sire":sireNum}).append(sireID))}},updateSireList:function(sireNum){
...
;if(0==$option.length)$("select.sireBreeding").append($("<option></option>",{value:sireNum,
"data-sire":sireNum}).append(sireID));else $option.text(sireID)},makeSireCardBody:function(sireNum){
...
this.deleteSire(sireNum)}),$body.find(".updateSireObj").on("change",e=>{var $el=$(e.target)
...
makeRow:function(label,$input,addRowClass=""){var myLeftCol="col-12 col-lg-6"
;if(addRowClass)addRowClass=" "+addRowClass;var $label=$("<label></label>",{for:$input.attr("id")
}).append(label),$row=$("<div></div>",{class:"row mt-2"+addRowClass}).append($("<div></div>",{
class:myLeftCol}).append($label)).append($("<div></div>",{class:"col"}).append($input));return $row
},addDamGeneration:function(damGenerationNum){
var inArray=this.checkInArray(damGenerationNum,this.damGenerationNums)
;if(!inArray)this.damGenerationNums.push(damGenerationNum),this.damGenerations[damGenerationNum]={
damGen:null,damGenDOB:null,damGenStrain:null},this.makeDamGenCard(damGenerationNum)},
makeDamGenCard:function(damGenerationNum){var $div=$("#damGensCardDiv")
;if(!$div.find(".card").length)$div.html("");var row="row mt-2",col="col-12 col-lg-6"
;$div.append($("<div/>",{class:"col col-md-6 mt-2 damGenCard","data-damgen":damGenerationNum}))
...
;this.makeCard($damDiv,header,$body),this.updateDamGenList(damGenerationNum)},
...
;genInfo=gen,$(".damGenCalc"+this.damGenSearch(damGenNum)).text(genInfo);var dob=damGen.damGenDOB
;if(dob)genInfo+="; DOB: "+dob;var strain=damGen.damGenStrain;if(strain)genInfo+="; strain: "+strain
...
makeDamGenCardBody:function(damGenerationNum){console.log("genNum:",damGenerationNum)
;var $body=$("<div></div>"),initialRows=[{label:"<h4>Generation:</h4>",type:"text",
className:"damGen",addRowClass:" updateDamGenerationObj"},{label:"Delete:",type:"button",
className:"deleteDamGeneration",optionsObj:[],addRowClass:" hideView"},{label:"DOB:",type:"date",
className:"damGenDOB",addRowClass:" updateDamGenerationObj"},{label:"Strain:",type:"text",
className:"damGenStrain",addRowClass:" updateDamGenerationObj"}]
...
this.sireGenerations[sireGenerationNum]={sireGen:null,sireGenDOB:null,sireGenStrain:null},
this.makeSireGenCard(sireGenerationNum)},makeSireGenCard:function(sireGenerationNum){
...
;$(".sireGeneration"+sireSearch).append($("<option></option>",{value:sireGenNum,
"data-siregen":sireGenNum}).append(genInfo))}},updateSireGenList:function(sireGenNum){
...
},{label:"Delete:",type:"button",className:"deleteSireGeneration",optionsObj:[],
addRowClass:" hideView"},{label:"DOB:",type:"date",className:"sireGenDOB",
addRowClass:" updateSireGenerationObj"},{label:"Strain:",type:"text",className:"sireGenStrain",
...
var lowerCaseName=className.toLowerCase();dataString="data-"+dataName.toLowerCase()
;var idNum=dataNum;if(addSecondData)secondDataString="data-"+secondDataName.toLowerCase(),
...
optionsObj))$input.append($("<option></option>",{value:option.value}).append(option.text))
}else if("textarea"===inputType){var inputObj={name:lowerCaseName+idNum,id:className+idNum,
...
this.updateTextarea(e.currentTarget)})}else{var inputObj={type:inputType,name:lowerCaseName+idNum,
id:className+idNum,class:className+" fullWidth watch","data-watch":className}
;if(inputObj[dataString]=dataNum,addSecondData)inputObj[secondDataString]=secondDataNum
...
this.checkDateFormat($(e))}).on("change",e=>{this.checkDateFormat($(e.currentTarget))})
;return $input},
...
var index=this.damNums.indexOf(damNum);if(index>-1)this.damNums.splice(index,1)
;delete this.dams[damNum];var damSearch=this.damSearch(damNum);$(damSearch).remove(),
this.getDamsDue($("#dueDate").val())}),this.resize()},deleteSire:function(sireNum){
this.runIfConfirmed("Are you sure that you wish to delete this sire?",()=>{
var index=this.sireNums.indexOf(sireNum);if(index>-1)this.sireNums.splice(index,1)
;delete this.sires[sireNum];var sireSearch=this.sireSearch(sireNum);$(sireSearch).remove(),
...
;$(damSearch+breedingSearch+plugSearch).remove(),this.getDamsDue($("#dueDate").val())}),
this.resize()},deleteMassFuncs:function(damNum,breedingNum,massNum){
this.runIfConfirmed("Are you sure that you wish to delete this dam breeding?",()=>{
...
;$(damSearch+breedingSearch+massSearch).remove(),this.getDamsDue($("#dueDate").val())}),
this.resize()},deleteDamGen:function(genNum){
...
$(damGenSearch).remove(),this.getDamsDue($("#dueDate").val())}),this.resize()},
...
;if(index>-1)this.sireGenerationNums.splice(index,1);delete this.sireGenerations[genNum]
;var sireGenSearch=this.sireGenSearch(genNum);$(".sireGenCard"+sireGenSearch).remove(),
$(sireGenSearch).remove(),this.getDamsDue($("#dueDate").val())}),this.resize()},
...
;if(!inArray)breedingNums.push(breedingNum),damBreedings[breedingNum]={sireBreeding:null,
litterNum:NaN,breedDate:NaN,breedCage:NaN,breedCageLoc:null,initialMass:NaN,massNums:[],masses:{},
plugCheckNums:[],plugChecks:{},sepFromMaleDate:NaN,sepDamCageNum:NaN,litterDOB:NaN,
stopTrackingDate:NaN},this.makeDamBreeding(damNum,breedingNum),this.getDamsDue($("#dueDate").val())
...
label:"Date:",type:"date",className:"breedDate",addRowClass:"updateDamBreedObj"},{
label:"Litter Number:",type:"number",className:"litterNum",addRowClass:"updateDamBreedObj"},{
label:"Cage Num:",type:"number",className:"breedCage",addRowClass:"updateDamBreedObj"},{
label:"Cage location:",type:"text",className:"breedCageLoc",addRowClass:"updateDamBreedObj"},{
label:"Initial mass:",type:"number",className:"initialMass",addRowClass:"updateDamBreedObj"},{
label:"Add plug check:",type:"button",className:"addPlugCheck",addRowClass:"hideView"},{
label:"Add mass:",type:"button",className:"addMass",addRowClass:"hideView"},{
label:"Separated from male:",type:"date",className:"sepFromMaleDate",addRowClass:"updateDamBreedObj"
},{label:"Sep cage #:",type:"number",className:"sepDamCageNum"},{label:"Parturition date:",
type:"date",className:"litterDOB",addRowClass:"updateDamBreedObj"},{label:"Stop tracking:",
type:"date",className:"stopTrackingDate",addRowClass:"updateDamBreedObj"},{label:"Copy info:",
type:"button",className:"copyBreeding"}];$cardBody.append($("<div></div>",{class:"breedDiv",
"data-breed":breedingNum,"data-dam":damNum}).append($("<hr></hr>")))
...
this.addPlugCheck(damNum,breedingNum)}).closest(".row").after($("<div></div>",{class:"plugsDiv",
"data-dam":damNum,"data-breed":breedingNum}).append(this.makeTopLabelRow(this.plugLabels))),
$body.find(".addMass").prop("value","Add mass").on("click",e=>{this.addMass(damNum,breedingNum)
}).closest(".row").after($("<div></div>",{class:"massesDiv","data-dam":damNum,
"data-breed":breedingNum
}).append(this.makeTopLabelRow(this.massLabels))),$body.find(".deleteBreeding").prop("value","Delete Breeding").on("click",e=>{
...
class:"card d-none d-md-flex topLabelRow"}).append($("<div></div>",{class:"row"
}).append($("<div></div>",{class:"col-12 row labelRow"
...
class:"col-12 font-weight-bold"}).append(label));var inputCols=[{type:"date",className:"plugDate"},{
type:"select",className:"plugCheck",optionsObj:[{value:"neg",text:"-/-"},{value:"1",text:"?"},{
value:"2",text:"+/-"},{value:"3",text:"+/+"},{value:"red",text:"Red"},{value:"closed",
text:"Closed VO"}]},{type:"textarea",className:"plugComments"},{type:"button",className:"deletePlug"
...
this.resize()},addMass:function(damNum,breedingNum){
...
makeMassEntry:function(damNum,breedingNum,massNum){var $card=$("<div></div>",{class:"card massCard",
"data-dam":damNum,"data-breed":breedingNum,"data-mass":massNum}).append($("<div></div>",{class:"row"
}).append($("<div></div>",{class:"col-6 d-md-none row labelRow"})).append($("<div></div>",{
class:"col-6 col-md-12 row massRow"})))
;for(label of this.massLabels)$card.find(".labelRow").append($("<div></div>",{
class:"col-12 font-weight-bold"}).append(label));var inputCols=[{type:"date",className:"massDate"},{
type:"number",className:"mass"},{type:"noEntry"},{type:"button",className:"deleteMass"}]
;for(colObj of inputCols)if("noEntry"===colObj.type)$card.find(".massRow").append($("<div></div>",{
class:"change col-12 col-md-"+Math.floor(12/inputCols.length),"data-dam":damNum,
"data-breed":breedingNum,"data-mass":massNum
...
$tableDiv.find("tr").each((i,e)=>{$(e).find("td, th").each((i,e)=>{if(2===i){
var DOBcertain=$(e).text()
;if($(e).addClass("hide"),"true"===DOBcertain)$(e).parent().addClass("isCertain");else $(e).parent().removeClass("isCertain")
}var text=$(e).text()
;if(this.isValidDate(text))$(e).text(luxon.DateTime.fromISO(text).toLocaleString({weekday:"short",
...
var sireInfo=this.sires[sireNum];if(sireID=sireInfo.sireID,sireGenNum=sireInfo.sireGeneration,
sireGenNum){var sireGenInfo=this.sireGenerations[sireGenNum];sireStrain=sireGenInfo.sireGenStrain,
sireGeneration=sireGenInfo.sireGen,sireDOB=sireGenInfo.sireGenDOB}}
var plugDate=breeding.potentialPlugDate,damObj={damID:specDamID,litterNum:breeding.litterNum,
dam:damID,damStrain:damStrain,damGeneration:damGeneration,damDOB:damDOB,sire:sireID,
sireStrain:sireStrain,sireGeneration:sireGeneration,sireDOB:sireDOB,breedDate:breeding.breedDate,
plugDate:plugDate,DOB:breeding.litterDOB,Sac_or_stop:breeding.stopTrackingDate};return damObj},
...
this.printDams(namesSeparation,$(".sepMaleList")),this.printDams(namesBirth,$(".checkBirthsList")),
this.makeSeparationTable(damsForSeparationObj.remaining),
this.makeCheckForBirthsTable(damsForBirthObj.remaining),this.makeEstDatesTable(),
this.makeDamsBreedingTable(),this.resize()},
...
}else if(val=bInfo[label],!val)val="";row.push(val)}tableData.push(row)}
$tableDiv=$(".sepDueDatesTable"),this.createTable(tableData,true,false,$tableDiv),
$tableDiv.find("td").each((i,e)=>{var text=$(e).text();if(this.isValidDate(text)){
...
;var tableBody=document.createElement("tbody");if(tableData.forEach((rowData,rowNum)=>{
...
this.dams[damNum].food=foodDate,this.dams[damNum].bottom=bottomDate,this.dams[damNum].top=topDate,{
water:waterDate,food:foodDate,bottom:bottomDate,top:topDate}},
...
;var dam=dams[damNum],endDate=dam.endDam;if(!endDate){var row=[];for(label of labels){
var val=dam[label];if(!val)val="";row.push(val)}tableData.push(row)}}$tableDiv=$(".dueDatesTable"),
...
;this.resize()},makeArrayFromStr:function(textStr){var results=Papa.parse(textStr,{
skipEmptyLines:true});return results.data},preview:function(){
...
sireGenLabels:["sireGen","sireGenDOB","sireGenStrain"],addDamGenFromTable:function(obj){
var genNum=this.getNextNum(this.damGenerationNums);this.addDamGeneration(genNum)
;var damGenerationSearch=this.damGenSearch(genNum),headers=this.damGenLabels
...
;var sireGenerationSearch=this.sireGenSearch(genNum),headers=this.sireGenLabels
...
},genNum=this.addDamGenFromTable(obj);$(".damGeneration"+damSearch).val(genNum)}
this.updateObjFromVal($(".damGeneration"+damSearch),this.dams[damNum])},
addSireFromTable:function(obj,headers){var sireNum=this.getNextNum(this.sireNums)
;this.addSire(sireNum);var sireSearch=this.sireSearch(sireNum);for(header of headers){
var sireGen,sireGenDOB,sireGenStrain;if("sireID"===header)$("."+header+sireSearch).val(obj[header]),
...
this.updateObjFromVal($(".sireGeneration"+sireSearch),this.sires[sireNum])},upload:function(){
var tableStr=this.addedTable;if(tableStr){var byHeader=Papa.parse(tableStr,{skipEmptyLines:true,
header:true}),dataByHeader=byHeader.data,headers=byHeader.meta.fields
...
;matchingObjs=matchThirdKeys}}else if(0===matchingObjs.length)noMatches=true
;if(1!==matchingObjs.length)matchingObj={};else matchingObj=matchingObjs[0];return{
...
var damInfo=this.dams[damNum],breedingNum=this.getDamLatestBreeding(damInfo,dueDate)
;if(breedingNum){var bInfo=damInfo.breedings[breedingNum]
...
!bInfo.likelyPlugDate||bInfo.likelyPlugDate>dueDate)damsForLikely.push(damNum)}}return{
remaining:damsToCheck,forLikely:damsForLikely}},
getDamsForSeparation:function(dueDate=luxon.DateTime.now().toISODate()){var damNums=this.damNums
;const damsToCheck=[],damsDueToCheck=[];for(damNum of damNums){
var damInfo=this.dams[damNum],breedingNum=this.getDamLatestBreeding(damInfo,dueDate)
;if(breedingNum){var bInfo=damInfo.breedings[breedingNum]
...
damNum:damNum,breedingNum:breedingNum}),bInfo.sepBreedingDate<=dueDate)damsDueToCheck.push(damNum)}}
return{remaining:damsToCheck,due:damsDueToCheck}},
getDamsForBirth:function(dueDate=luxon.DateTime.now().toISODate()){var damNums=this.damNums
;const damsToCheck=[],damsDueToCheck=[];for(damNum of damNums){
var damInfo=this.dams[damNum],breedingNum=this.getDamLatestBreeding(damInfo,dueDate)
;if(breedingNum){var bInfo=damInfo.breedings[breedingNum]
;if((!bInfo.litterDOB||bInfo.litterDOB>dueDate)&&(!bInfo.stopTrackingDate||bInfo.stopTrackingDate>dueDate))if(damsToCheck.push({
damNum:damNum,breedingNum:breedingNum}),bInfo.birthBreedingDate<=dueDate)damsDueToCheck.push(damNum)
}}return{remaining:damsToCheck,due:damsDueToCheck}},
