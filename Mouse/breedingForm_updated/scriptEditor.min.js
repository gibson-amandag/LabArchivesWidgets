my_widget_script={damGenerationNums:[],damGenerations:{},
sireGenerationNums:[],sireGenerations:{},damNums:[],dams:{},sireNums:[],
sires:{},mode:"edit",init:function(mode,json_data){
...
script.onload=script.onreadystatechange=function(){if(script.readyState){
...
this.setUpInitialState(),this.adjustForMode(mode),this.checkForNames()},
to_json:function(){
...
sireGenerations:this.sireGenerations,damNums:this.damNums,dams:this.dams,
sireNums:this.sireNums,sires:this.sires};return JSON.stringify(output)},
from_json:function(json_data){var parsedJson=JSON.parse(json_data)
;this.parent_class.from_json(JSON.stringify(parsedJson.widgetData))},
test_data:function(){
var testData=JSON.parse(this.parent_class.test_data()),output={
widgetData:testData};return JSON.stringify(output)},
is_valid:function(b_suppress_message){var fail=false,fail_log="",name
...
}}),$("input[type='time']").each((i,e)=>{var time=$(e).val();if(time){
var validtime=this.isValidTime(time)
;if(!validtime)fail=true,fail_log+="Please enter valid time in form 'hh:mm' - 24 hr time"
}}),fail)return bootbox.alert(fail_log);else{var noErrors=[]
;return noErrors}},is_edited:function(){
return this.parent_class.is_edited()},reset_edited:function(){
return this.parent_class.reset_edited()},parseInitJson:function(json_data){
var jsonString
;if("string"===typeof json_data)jsonString=json_data;else jsonString=json_data()
;var parsedJson=JSON.parse(jsonString);return parsedJson},
...
if(parsedJson.sires)this.sires=parsedJson.sires;if(parsedJson.sireNums){
for(sireNum of parsedJson.sireNums)this.makeSireCard(sireNum)
;this.sireNums=parsedJson.sireNums}
if(parsedJson.dams)this.dams=parsedJson.dams;if(parsedJson.damNums){
for(damNum of parsedJson.damNums){this.makeDamCard(damNum)
;var breedingNums=this.dams[damNum].breedingNums
;if(breedingNums)for(breedingNum of breedingNums){
this.makeDamBreeding(damNum,breedingNum)
;var plugCheckNums=this.dams[damNum].breedings[breedingNum].plugCheckNums
;if(plugCheckNums)for(plugCheckNum of plugCheckNums)this.makePlugEntry(damNum,breedingNums,plugCheckNum)
;var massNums=this.dams[damNum].breedings[breedingNum].massNums
;if(massNums)for(massNum of massNums)this.makeMassEntry(damNum,breedingNums,massNum)
...
$(".hideView").hide(),$("#dueDate").datepicker({dateFormat:"yy-mm-dd"
}),$("#careDate").datepicker({dateFormat:"yy-mm-dd"
});else $("input[type='date']").each((i,e)=>{this.checkDateFormat($(e))
}),$("input[type='time']").each((i,e)=>{this.checkTimeFormat($(e))})},
addEventListeners:function(){},addRequiredFieldIndicators:function(){
...
isTimeSupported:function(){var input=document.createElement("input")
;input.setAttribute("type","time");var supported=true
;if("time"!==input.type)supported=false
;return this.timeSupported=supported,input.remove(),supported},
timeSupported:true,checkTimeFormat:function($timeInput){
if(!this.timeSupported){$timeInput.next(".timeWarning").remove()
...
var regEx=/^\d{4}-\d{2}-\d{2}$/;if(!dateString.match(regEx))return false
;var d=new Date(dateString),dNum=d.getTime()
;if(!dNum&&0!==dNum)return false
;return d.toISOString().slice(0,10)===dateString},
isDateSupported:function(){var input=document.createElement("input")
;input.setAttribute("type","date");var supported=true
;if("date"!==input.type)supported=false
;return this.dateSupported=supported,input.remove(),supported},
dateSupported:true,checkDateFormat:function($dateInput){
if(!this.dateSupported){$dateInput.next(".dateWarning").remove()
...
;$dateInput.datepicker({dateFormat:"yy-mm-dd"}),this.resize()}},
...
this.checkTimeFormat($(e.target))}),$("textarea.autoAdjust").each((i,e)=>{
...
;this.addDamGeneration(damGeneration)}),$(".addSireGen").on("click",e=>{
...
}),$("#collapseDams").on("click",e=>{this.collapseAllDamCards()
}),$("#the_form").on("input",e=>{
if($(e.target).data("watch"))this.watchValue($(e.target));else this.updateCalcFromEl(e.target)
}).on("change",e=>{this.getDamsDue($("#dueDate").val())
...
;$("#dueDate").val(dateToday).each((i,e)=>{this.getDamsDue($(e).val())
}),$(".htmlCardHeader").on("click",e=>{this.toggleCard($(e.currentTarget))
...
}),$("#preview").on("click",e=>{this.preview()}),this.resize()},
...
;this.resize()},getCheckState:function($el){var checkState=false
;if($el.is(":checked"))checkState=true;return checkState},
resize:function(){this.parent_class.resize_container()},
checkForNames:function(){$("input, select, textarea").each((i,e)=>{
...
;return dynamicContent},data_valid_form:function($errorMsg){
...
var thisMessage="Are you sure?";if(text)thisMessage=text;var top="auto"
;if(elForHeight)top=elForHeight.offsetTop+"px";bootbox.confirm({
message:thisMessage,callback:proceed=>{if(proceed)functionToCall()}
}),$(".modal-dialog").css("top",top)},
dialogConfirm:function(text,functionToCall,elForHeight=null){
var thisMessage="Do you want to proceed?";if(text)thisMessage=text
;var top="auto";if(elForHeight)top=elForHeight.offsetTop+"px"
;bootbox.confirm({message:thisMessage,callback:result=>{
functionToCall(result)}}),$(".modal-dialog").css("top",top)},
runBasedOnInput:function(prompt,functionToCall,elForHeight=null){
var thisTitle="Enter value:";if(prompt)thisTitle=prompt;var top="auto"
;if(elForHeight)top=elForHeight.offsetTop+"px";bootbox.prompt({
title:thisTitle,callback:result=>{functionToCall(result)}
}),$(".modal-dialog").css("top",top)},
checkInArray:function(searchVal,array){
var proceed=-1!==$.inArray(searchVal,array);return proceed},
dataSearch:function(dataName,dataValue){
var dataSearch="[data-"+dataName+"='"+dataValue+"']";return dataSearch},
tableSearch:function(table){var tableSearch=this.dataSearch("table",table)
;return tableSearch},calcSearch:function(calc){
var calcSearch=this.dataSearch("calc",calc);return calcSearch},
daySearch:function(day){var daySearch=this.dataSearch("day",day)
;return daySearch},mouseSearch:function(mouse){
var mouseSearch=this.dataSearch("mouse",mouse);return mouseSearch},
damSearch:function(dam){var damSearch=this.dataSearch("dam",dam)
;return damSearch},sireSearch:function(sire){
var sireSearch=this.dataSearch("sire",sire);return sireSearch},
damGenSearch:function(damGen){
var damGenSearch=this.dataSearch("damgen",damGen);return damGenSearch},
sireGenSearch:function(sireGen){
var sireGenSearch=this.dataSearch("siregen",sireGen);return sireGenSearch},
breedingSearch:function(breedingNum){
var breedingSearch=this.dataSearch("breed",breedingNum)
;return breedingSearch},plugSearch:function(plugNum){
var plugSearch=this.dataSearch("plug",plugNum);return plugSearch},
massSearch:function(massNum){var massSearch=this.dataSearch("mass",massNum)
;return massSearch},updateCalcFromEl:function(el){
var calc=el.id,val=el.value,calcSearch=this.calcSearch(calc)
;$(calcSearch).html(val),this.resize()},
updateCalcFromVal:function(calc,val){var calcSearch=this.calcSearch(calc)
;$(calcSearch).text(val),this.resize()},watchValue:function($el){
...
this.data_valid_form($errorMsg),$table.toggle(),this.resize()},
...
},downloadCSV:function(csv,filename){var csvFile,downloadLink
...
if(transpose)rowNum=0;if($(e).find("td, th").each((i,e)=>{
...
;var tableString=rowString.join(newRowString);return tableString},
...
$temp.remove(),this.resize()},toggleCard:function($cardHead){
...
$div.append($("<div></div>",{class:"card"}).append($("<button></button>",{
type:"button",class:"card-header"}).on("click",e=>{
this.toggleCard($(e.currentTarget))
}).append(cardHeadContent)).append($("<div></div>",{
class:"card-body collapse"}).append(cardBodyContent))),this.resize()},
addDam:function(damNum){var inArray=this.checkInArray(damNum,this.damNums)
;if(!inArray)this.damNums.push(damNum),this.dams[damNum]={damID:"",
damGeneration:"",breedingNums:[],breedings:{}},this.makeDamCard(damNum)},
makeDamCard:function(damNum){var $div=$("#damCardDiv")
;if(!$div.find(".card").length)$div.html("")
;var row="row mt-2",col="col-12 col-lg-6";$div.append($("<div/>",{
class:"col-12 col-lg-6 mt-2 damCard","data-dam":damNum}))
...
value:damNum,class:"damIDCalc","data-calc":"damID","data-dam":damNum
}).append("Dam "+damNum)).attr("size",Math.min(this.damNums.length,15))},
makeDamCardBody:function(damNum){var $body=$("<div></div>"),initialRows=[{
label:"<h4>Dam ID:</h4>",type:"text",className:"damID",
addRowClass:" updateDamObj"},{label:"Delete:",type:"button",
className:"deleteDam",optionsObj:[],addRowClass:" hideView"},{
label:"Generation:",type:"select",className:"damGeneration",
addRowClass:" updateDamObj",optionsObj:[{value:"",text:"[Select]"}]},{
label:"Add breeding:",type:"button",className:"addBreeding"}]
;for(row of initialRows)$body.append(this.makeRowFromObj(row,damNum,"dam"))
;return $body.find(".deleteDam").prop("value","Delete Dam").on("click",e=>{
this.deleteDamFuncs(damNum)
}),$body.find(".addBreeding").prop("value","Add Breeding").on("click",e=>{
this.addDamBreeding(damNum)}),$body.find(".updateDamObj").on("change",e=>{
var $el=$(e.target),val=$el.val();valSafe=this.encodeHTML(val)
;var thisProp=$el.data("watch");this.dams[damNum][thisProp]=valSafe}),$body
},addSire:function(sireNum){
var inArray=this.checkInArray(sireNum,this.sireNums)
;if(!inArray)this.sireNums.push(sireNum),this.sires[sireNum]={sireID:"",
sireGeneration:""},this.makeSireCard(sireNum)},
makeSireCard:function(sireNum){var $div=$("#sireCardDiv")
;if(!$div.find(".card").length)$div.html("")
;var row="row mt-2",col="col-12 col-lg-6";$div.append($("<div/>",{
class:"col col-md-6 mt-2 sireCard","data-sire":sireNum}))
...
value:sireNum,class:"sireIDCalc","data-calc":"sireID","data-sire":sireNum
}).append("Sire "+sireNum)).attr("size",Math.min(this.sireNums.length,15)),
this.updateSireList(sireNum)},makeSireList:function(damNum,damBreedingNum){
var sires=this.sires,sireNums=this.sireNums,damSearch=this.damSearch(damNum),breedingSearch=this.breedingSearch(damBreedingNum),damBreedingSearch=damSearch+breedingSearch
;for(sireNum of sireNums){
var sire=sires[sireNum],sireInfo="",sireID=sire.sireID
;if(!sireID)sireID="Sire "+sireNum
;$("select.sireBreeding"+damBreedingSearch).append($("<option></option>",{
value:sireNum,"data-sire":sireNum}).append(sireID))}},
updateSireList:function(sireNum){
var sires=this.sires,sire=sires[sireNum],sireInfo="",sireID=sire.sireID
;if(!sireID)sireID="Sire "+sireNum
;var $option=$("select.sireBreeding").find("option[value='"+sireNum+"']")
;if(0==$option.length)$("select.sireBreeding").append($("<option></option>",{
value:sireNum,"data-sire":sireNum
}).append(sireID));else $option.text(sireID)},
setSelectizeValue:function(elName,value){
$("elName").val(value),$jq351("elName")[0].selectize.setValue(value)},
makeSireCardBody:function(sireNum){
var $body=$("<div></div>"),initialRows=[{label:"<h4>Sire ID:</h4>",
type:"text",className:"sireID",addRowClass:" updateSireObj"},{
label:"Delete:",type:"button",className:"deleteSire",optionsObj:[],
addRowClass:" hideView"},{label:"Generation:",type:"select",
className:"sireGeneration",addRowClass:" updateSireObj",optionsObj:[{
value:"",text:"[Select]"}]}]
;for(row of initialRows)$body.append(this.makeRowFromObj(row,sireNum,"sire"))
;return $body.find(".deleteSire").prop("value","Delete Sire").on("click",e=>{
...
this.updateSireList(sireNum)}),$body},encodeHTML:function(dirtyString){
var container=document.createElement("div"),text=document.createTextNode(dirtyString)
;return container.appendChild(text),container.innerHTML},
makeRow:function(label,$input,addRowClass=""){
var myLeftCol="col-12 col-lg-6";if(addRowClass)addRowClass=" "+addRowClass
;var $label=$("<label></label>",{for:$input.attr("id")
}).append(label),$row=$("<div></div>",{class:"row mt-2"+addRowClass
}).append($("<div></div>",{class:myLeftCol
}).append($label)).append($("<div></div>",{class:"col"}).append($input))
;return $row},addDamGeneration:function(damGenerationNum){
...
this.damGenerations[damGenerationNum]={damGen:null,damGenDOB:null,
damGenStrain:null},this.makeDamGenerationCard(damGenerationNum)},
makeDamGenerationCard:function(damGenerationNum){
var $div=$("#damGensCardDiv");if(!$div.find(".card").length)$div.html("")
;var row="row mt-2",col="col-12 col-lg-6";$div.append($("<div/>",{
...
class:"damGenCalc","data-calc":"damGen","data-damgen":damGenerationNum
}).append("Dam Generation "+damGenerationNum),$body=this.makeDamGenerationCardBody(damGenerationNum)
;this.makeCard($damDiv,header,$body),
this.updateDamGenList(damGenerationNum)},makeDamGenList:function(damNum){
...
;if(!gen)gen="Gen "+damGenNum;genInfo=gen;var dob=damGen.damGenDOB
;if(dob)genInfo+="; DOB: "+dob;var strain=damGen.damGenStrain
;if(strain)genInfo+="; strain: "+strain
;$(".damGeneration"+damSearch).append($("<option></option>",{
value:damGenNum,"data-damgen":damGenNum}).append(genInfo))}},
...
;var dob=damGen.damGenDOB;if(dob)genInfo+="; DOB: "+dob
...
;var $body=$("<div></div>"),initialRows=[{label:"<h4>Generation:</h4>",
type:"text",className:"damGen",addRowClass:" updateDamGenerationObj"},{
label:"Delete:",type:"button",className:"deleteDamGeneration",
optionsObj:[],addRowClass:" hideView"},{label:"DOB:",type:"date",
className:"damGenDOB",addRowClass:" updateDamGenerationObj"},{
label:"Strain:",type:"text",className:"damGenStrain",
...
this.sireGenerations[sireGenerationNum]={sireGen:null,sireGenDOB:null,
sireGenStrain:null},this.makeSireGenerationCard(sireGenerationNum)},
makeSireGenerationCard:function(sireGenerationNum){
var $div=$("#sireGensCardDiv");if(!$div.find(".card").length)$div.html("")
;var row="row mt-2",col="col-12 col-lg-6";$div.append($("<div/>",{
...
;if(!gen)gen="Gen "+sireGenNum;genInfo=gen;var dob=sireGen.sireGenDOB
;if(dob)genInfo+="; DOB: "+dob;var strain=sireGen.sireGenStrain
;if(strain)genInfo+="; strain: "+strain
;$(".sireGeneration"+sireSearch).append($("<option></option>",{
value:sireGenNum,"data-siregen":sireGenNum}).append(genInfo))}},
...
;var $body=$("<div></div>"),initialRows=[{label:"<h4>Generation:</h4>",
type:"text",className:"sireGen",addRowClass:" updateSireGenerationObj"},{
label:"Delete:",type:"button",className:"deleteSireGeneration",
optionsObj:[],addRowClass:" hideView"},{label:"DOB:",type:"date",
className:"sireGenDOB",addRowClass:" updateSireGenerationObj"},{
label:"Strain:",type:"text",className:"sireGenStrain",
...
;dataString="data-"+dataName.toLowerCase();var idNum=dataNum
...
idNum+="c"+thirdDataNum;if("select"===inputType){var selectObj={
name:lowerCaseName+idNum,id:className+idNum,
class:className+" fullWidth watch","data-watch":className}
;if(selectObj[dataString]=dataNum,
...
optionsObj))$input.append($("<option></option>",{value:option.value
}).append(option.text))}else if("textarea"===inputType){var inputObj={
name:lowerCaseName+idNum,id:className+idNum,
class:className+" fullWidth watch autoAdjust","data-watch":className}
;if(inputObj[dataString]=dataNum,
addSecondData)inputObj[secondDataString]=secondDataNum
;if(addThirdData)inputObj[thirdDataString]=thirdDataNum
;$input=$("<tex"+"tarea></tex"+"tarea>",inputObj).on("input",e=>{
this.updateTextarea(e.currentTarget)})}else{var inputObj={type:inputType,
name:lowerCaseName+idNum,id:className+idNum,
class:className+" fullWidth watch","data-watch":className}
;if(inputObj[dataString]=dataNum,
addSecondData)inputObj[secondDataString]=secondDataNum
...
if("time"===inputType)$input.each((i,e)=>{this.checkTimeFormat($(e))
}).on("change",e=>{this.checkTimeFormat($(e.currentTarget))})
;if("date"===inputType)$input.each((i,e)=>{this.checkDateFormat($(e))
}).on("change",e=>{this.checkDateFormat($(e.currentTarget))});return $input
},
...
;if(index>-1)this.damNums.splice(index,1);delete this.dams[damNum]
;var damSearch=this.damSearch(damNum)
;$(".damCard"+damSearch).remove(),$(damSearch).remove(),
this.getDamsDue($("#dueDate").val())}),this.resize()},
...
var thisDam=this.dams[damNum],damBreedingNums=thisDam.breedingNums,damBreedings=thisDam.breedings,index=damBreedingNums.indexOf(damBreedingNum)
;if(index>-1)damBreedingNums.splice(index,1)
;delete damBreedings[damBreedingNum]
;var damSearch=this.damSearch(damNum),breedingSearch=this.breedingSearch(damBreedingNum)
;$(damSearch+breedingSearch).remove(),this.getDamsDue($("#dueDate").val())
}),this.resize()},
deletePlugFuncs:function(damNum,damBreedingNum,plugCheckNum){
this.runIfConfirmed("Are you sure that you wish to delete this dam breeding?",()=>{
var thisDam=this.dams[damNum],damBreeding=thisDam.breedings[damBreedingNum],plugCheckNums=damBreeding.plugCheckNums,plugChecks=damBreeding.plugChecks,index=plugCheckNums.indexOf(plugCheckNum)
;if(index>-1)plugCheckNums.splice(index,1);delete plugChecks[plugCheckNum]
;var damSearch=this.damSearch(damNum),breedingSearch=this.breedingSearch(damBreedingNum),plugSearch=this.plugSearch(plugCheckNum)
;$(damSearch+breedingSearch+plugSearch).remove(),
this.getDamsDue($("#dueDate").val())}),this.resize()},
deleteMassFuncs:function(damNum,damBreedingNum,massNum){
this.runIfConfirmed("Are you sure that you wish to delete this dam breeding?",()=>{
var thisDam=this.dams[damNum],damBreeding=thisDam.breedings[damBreedingNum],massNums=damBreeding.massNums,masses=damBreeding.masses,index=massNums.indexOf(massNum)
;if(index>-1)massNums.splice(index,1);delete masses[massNum]
;var damSearch=this.damSearch(damNum),breedingSearch=this.breedingSearch(damBreedingNum),massSearch=this.massSearch(massNum)
...
$(damGenSearch).remove(),this.getDamsDue($("#dueDate").val())
}),this.resize()},addDamBreeding:function(damNum){
var damBreedingNums=this.dams[damNum].breedingNums,damBreedings=this.dams[damNum].breedings
;if(damBreedingNums.length>0)var lastDamBreeding=damBreedingNums[damBreedingNums.length-1],damBreedingNum=lastDamBreeding+1;else var damBreedingNum=1
;var inArray=this.checkInArray(damBreedingNum,damBreedingNums)
;if(!inArray)damBreedingNums.push(damBreedingNum),
damBreedings[damBreedingNum]={sireBreeding:null,litterNum:NaN,
breedDate:NaN,breedCage:NaN,breedCageLoc:null,initialMass:NaN,massNums:[],
masses:{},plugCheckNums:[],plugChecks:{},sepFromMaleDate:NaN,
sepDamCageNum:NaN,litterDOB:NaN,stopTrackingDate:NaN
...
label:"Copy info:",type:"button",className:"copyBreeding"},{
label:"Delete:",type:"button",className:"deleteBreeding",optionsObj:[],
addRowClass:"hideView"},{label:"Sire:",type:"select",
className:"sireBreeding",addRowClass:"updateDamBreedObj",optionsObj:[{
value:"",text:"[Select]"}]},{label:"Date:",type:"date",
className:"breedDate",addRowClass:"updateDamBreedObj"},{
label:"Litter Number:",type:"number",className:"litterNum",
addRowClass:"updateDamBreedObj"},{label:"Cage Num:",type:"number",
className:"breedCage",addRowClass:"updateDamBreedObj"},{
label:"Cage location:",type:"text",className:"breedCageLoc",
addRowClass:"updateDamBreedObj"},{label:"Initial mass:",type:"number",
className:"initialMass",addRowClass:"updateDamBreedObj"},{
label:"Add plug check:",type:"button",className:"addPlugCheck",
addRowClass:"hideView"},{label:"Add mass:",type:"button",
className:"addMass",addRowClass:"hideView"},{label:"Separated from male:",
type:"date",className:"sepFromMaleDate",addRowClass:"updateDamBreedObj"},{
label:"Sep cage #:",type:"number",className:"sepDamCageNum"},{
label:"Parturition date:",type:"date",className:"litterDOB",
addRowClass:"updateDamBreedObj"},{label:"Stop tracking:",type:"date",
className:"stopTrackingDate",addRowClass:"updateDamBreedObj"},{
label:"Copy info:",type:"button",className:"copyBreeding"}]
;$cardBody.append($("<div></div>",{class:"breedDiv",
"data-breed":damBreedingNum,"data-dam":damNum}).append($("<hr></hr>")))
;var $body=$cardBody.find(".breedDiv").last()
;for(row of initialRows)$body.append(this.makeRowFromObj(row,damNum,"dam",true,damBreedingNum,"breed"))
;$body.find(".addPlugCheck").prop("value","Plug check").on("click",e=>{
this.addPlugCheck(damNum,damBreedingNum)
}).closest(".row").after($("<div></div>",{class:"plugsDiv",
"data-dam":damNum,"data-breed":damBreedingNum
...
}).closest(".row").after($("<div></div>",{class:"massesDiv",
"data-dam":damNum,"data-breed":damBreedingNum
...
var $el=$(e.target),val=$el.val();valSafe=this.encodeHTML(val)
;var thisProp=$el.data("watch")
;if(this.dams[damNum].breedings[damBreedingNum][thisProp]=valSafe,
"initialMass"===thisProp)this.calcMassChange(damNum,damBreedingNum)
;if("breedDate"===thisProp)this.updateBreedingWatchDates(damNum,damBreedingNum)
}),this.resize()},plugLabels:["Date","Plug","Comments","Delete"],
massLabels:["Date","Mass","% initial mass","Delete"],
makeTopLabelRow:function(labels){var $cardDiv=$("<div></div>",{
class:"card d-none d-md-flex topLabelRow"}).append($("<div></div>",{
class:"row"}).append($("<div></div>",{class:"col-12 row labelRow"
}))),numLabels=labels.length,colText="col-"+Math.floor(12/numLabels)
;for(label of labels)$cardDiv.find(".labelRow").append($("<div></div>",{
class:"font-weight-bold "+colText}).append(label));return $cardDiv},
addPlugCheck:function(damNum,damBreedingNum){
var damInfo=this.dams[damNum],breedingInfo=damInfo.breedings[damBreedingNum],plugCheckNums=breedingInfo.plugCheckNums,plugChecks=breedingInfo.plugChecks
;if(plugCheckNums.length>0)var lastPlugCheckNum=plugCheckNums[plugCheckNums.length-1],plugCheckNum=lastPlugCheckNum+1;else var plugCheckNum=1
;var inArray=this.checkInArray(plugCheckNum,plugCheckNums)
;if(!inArray)plugCheckNums.push(plugCheckNum),plugChecks[plugCheckNum]={
plugDate:NaN,plugCheck:null,plugComments:null
},this.makePlugEntry(damNum,damBreedingNum,plugCheckNum)},
makePlugEntry:function(damNum,damBreedingNum,plugNum){
var $card=$("<div></div>",{class:"card plugCard","data-dam":damNum,
"data-breed":damBreedingNum,"data-plug":plugNum}).append($("<div></div>",{
class:"row"}).append($("<div></div>",{class:"col-6 d-md-none row labelRow"
})).append($("<div></div>",{class:"col-6 col-md-12 row plugRow"})))
;for(label of this.plugLabels)$card.find(".labelRow").append($("<div></div>",{
class:"col-12 font-weight-bold"}).append(label));var inputCols=[{
type:"date",className:"plugDate"},{type:"select",className:"plugCheck",
optionsObj:[{value:"neg",text:"-/-"},{value:"1",text:"?"},{value:"2",
text:"+/-"},{value:"3",text:"+/+"},{value:"red",text:"Red"},{
value:"closed",text:"Closed VO"}]},{type:"textarea",
className:"plugComments"},{type:"button",className:"deletePlug"}]
;for(colObj of inputCols)$card.find(".plugRow").append($("<div></div>",{
class:"col-12 col-md-"+Math.floor(12/inputCols.length)
}).append(this.makeInput(colObj.type,colObj.className,damNum,colObj.optionsObj,"dam",true,damBreedingNum,"breed",true,plugNum,"plug")))
;$card.find(".deletePlug").prop("value","Delete").on("click",e=>{
this.deletePlugFuncs(damNum,damBreedingNum,plugNum)
}),$card.find(".plugRow").on("change",e=>{var $el=$(e.target),val=$el.val()
...
this.resize()},addMass:function(damNum,damBreedingNum){
var damInfo=this.dams[damNum],breedingInfo=damInfo.breedings[damBreedingNum],massNums=breedingInfo.massNums,masses=breedingInfo.masses
;if(massNums.length>0)var lastMassNum=massNums[massNums.length-1],massNum=lastMassNum+1;else var massNum=1
...
var $card=$("<div></div>",{class:"card massCard","data-dam":damNum,
"data-breed":damBreedingNum,"data-mass":massNum}).append($("<div></div>",{
class:"row"}).append($("<div></div>",{class:"col-6 d-md-none row labelRow"
})).append($("<div></div>",{class:"col-6 col-md-12 row massRow"})))
;for(label of this.massLabels)$card.find(".labelRow").append($("<div></div>",{
class:"col-12 font-weight-bold"}).append(label));var inputCols=[{
type:"date",className:"massDate"},{type:"number",className:"mass"},{
...
"data-dam":damNum,"data-breed":damBreedingNum,"data-mass":massNum
}).append("Enter new mass"));else $card.find(".massRow").append($("<div></div>",{
class:"col-12 col-md-"+Math.floor(12/inputCols.length)
}).append(this.makeInput(colObj.type,colObj.className,damNum,colObj.optionsObj,"dam",true,damBreedingNum,"breed",true,massNum,"mass")))
;$card.find(".deleteMass").prop("value","Delete").on("click",e=>{
this.deleteMassFuncs(damNum,damBreedingNum,massNum)
}),$card.find(".massRow").on("change",e=>{var $el=$(e.target),val=$el.val()
;valSafe=this.encodeHTML(val);var thisProp=$el.data("watch")
;this.dams[damNum].breedings[damBreedingNum].masses[massNum][thisProp]=valSafe
})
;var damSearch=this.damSearch(damNum),breedingSearch=this.breedingSearch(damBreedingNum),massSearch=this.massSearch(massNum),searchString=damSearch+breedingSearch+massSearch
;$card.find(".mass").on("input",e=>{
var $el=$(e.currentTarget),newMass=$el.val(),initialMass=this.dams[damNum].breedings[damBreedingNum].initialMass,massText=this.calcPercMass(newMass,initialMass)
;$(".change"+searchString).text(massText)
}),$(".massesDiv"+damSearch+breedingSearch).append($card),this.resize()},
...
;var initialMass=breedingInfo.initialMass,damSearch=this.damSearch(damNum),breedingSearch=this.breedingSearch(damBreedingNum)
;for(massNum of breedingInfo.massNums){
var massSearch=this.massSearch(massNum),newMass=breedingInfo.masses[massNum].mass,massText=this.calcPercMass(newMass,initialMass)
;$(".change"+damSearch+breedingSearch+massSearch).text(massText)}},
calcPercMass:function(newMass,initMass){
var newMassVal=parseFloat(newMass),initMassVal=parseFloat(initMass),text
;if(newMassVal>0&&initMassVal>0){var percChange=newMassVal/initMassVal*100
;text=percChange.toFixed(1)
}else if(!newMassVal>0)text="Enter new mass";else text="Enter initial mass"
;return text},makeEstDatesTable:function(){
var damObjs=this.getDamsForImpDates(),labels=this.damDatesLabels
;const tableData=[labels];for(damObj of damObjs){
var damNum=damObj.damNum,damBreedingNum=damObj.damBreedingNum,datesInfo=this.getDamDates(damNum,damBreedingNum),row=[],val
;for(label of labels){if(val=datesInfo[label],!val)val="";row.push(val)}
tableData.push(row)}
$tableDiv=$(".estExpDatesTable"),this.createTable(tableData,true,false,$tableDiv),
$tableDiv.find("tr").each((i,e)=>{$(e).find("td, th").each((i,e)=>{
if(2===i){var DOBcertain=$(e).text()
;if($(e).addClass("hide"),"true"===DOBcertain)$(e).parent().addClass("isCertain");else $(e).parent().removeClass("isCertain")
}var text=$(e).text();if(this.isValidDate(text)){
if(text<=luxon.DateTime.now().toISODate())$(e).addClass("isDue")
;$(e).text(luxon.DateTime.fromISO(text).toLocaleString({weekday:"short",
month:"short",day:"2-digit"}))}})})},makeDamsBreedingTable:function(){
var dams=this.dams,damNums=this.damNums,labels=this.damBreedingLabels
;const tableData=[labels];for(damNum of damNums){
var damInfo=dams[damNum],damBreedingNums=damInfo.breedingNums
;for(damBreedingNum of damBreedingNums){
var breedingInfo=this.getDamBreedingInfo(damNum,damBreedingNum),row=[],val
;for(label of labels){if(val=breedingInfo[label],!val)val="";row.push(val)}
tableData.push(row)}}
$tableDiv=$(".damsBreedingsInfoTable"),this.createTable(tableData,true,false,$tableDiv)
},getDamDates:function(damNum,damBreedingNum){
var dam=this.dams[damNum],breeding=this.dams[damNum].breedings[damBreedingNum],damID=dam.damID
;if(!damID)damID="d"+damNum;var litterNum=breeding.litterNum
;if(!litterNum)litterNum=damBreedingNum
...
damID:specDamID,DOB:DOB,DOBcertain:DOBcertain,P4:P4,P11:P11,P21:P21,
P70:P70,P91:P91};return damObj},
...
;if(damGenNum){var damGenInfo=this.damGenerations[damGenNum]
;damStrain=damGenInfo.damGenStrain,
damGeneration=damGenInfo.damGen,damDOB=damGenInfo.damGenDOB}
var sireNum=breeding.sireBreeding,sireID,sireStrain,sireGeneration,sireDOB
;if(sireNum){var sireInfo=this.sires[sireNum]
;if(sireID=sireInfo.sireID,sireGenNum=sireInfo.sireGeneration,sireGenNum){
...
var plugDate=breeding.earliestPotentialPlug,damObj={damID:specDamID,
litterNum:breeding.litterNum,dam:damID,damStrain:damStrain,
damGeneration:damGeneration,damDOB:damDOB,sire:sireID,
sireStrain:sireStrain,sireGeneration:sireGeneration,sireDOB:sireDOB,
breedDate:breeding.breedDate,plugDate:plugDate,DOB:breeding.litterDOB}
;return damObj},
damBreedingLabels:["damID","litterNum","dam","damStrain","damGeneration","damDOB","sire","sireStrain","sireGeneration","sireDOB","breedDate","plugDate","DOB"],
...
;for(label of this.damBreedingLabels){var val=damObj[label];if(!val)val=""
...
this.makeEstDatesTable(),this.makeDamsBreedingTable(),this.resize()},
...
var dams=this.dams,labels=this.sepTableLabels
;const tableData=[this.sepTableHeaders]
;for(damBreedingObj of toBeSeparated){
var damNum=damBreedingObj.damNum,damBreedingNum=damBreedingObj.damBreedingNum,damInfo=dams[damNum],breedingInfo=damInfo.breedings[damBreedingNum],row=[],val
;for(label of labels){if("damID"==label){
if(val=damInfo[label],!val)val="Dam "+damNum
}else if(val=breedingInfo[label],!val)val="";row.push(val)}
tableData.push(row)}
$tableDiv=$(".sepDueDatesTable"),this.createTable(tableData,true,false,$tableDiv),
$tableDiv.find("td").each((i,e)=>{var text=$(e).text()
...
var dams=this.dams,labels=this.birthTableLabels
...
tableData.push(row)}
$tableDiv=$(".checkForBirthsTable"),this.createTable(tableData,true,false,$tableDiv),
$tableDiv.find("td").each((i,e)=>{var text=$(e).text()
;if(this.isValidDate(text)){
if(text<=luxon.DateTime.fromISO($("#dueDate").val()).toISODate())$(e).addClass("isDue")
;$(e).text(luxon.DateTime.fromISO(text).toLocaleString({weekday:"short",
month:"short",day:"2-digit"}))}})},getDamNamesArray:function(damNumArray){
var names=[];for(damNum of damNumArray){var thisID=this.dams[damNum].damID
;if(!thisID)thisID="Dam "+damNum;names.push(thisID)}return names},
...
var newDate=luxon.DateTime.fromISO($startDateVal).plus({days:numDays
}).toISODate();return newDate},
...
var row=document.createElement("tr");rowData.forEach((cellData,colNum)=>{
...
$tableDiv.append(table);this.resize()},getDueDatesForDam:function(damNum){
...
this.dams[damNum].top=topDate,{water:waterDate,food:foodDate,
bottom:bottomDate,top:topDate}},
...
;const tableData=[labels];for(damNum of damNums){
this.getDueDatesForDam(damNum);var dam=dams[damNum],endDate=dam.endDam
;if(!endDate){var row=[];for(label of labels){var val=dam[label]
;if(!val)val="";row.push(val)}tableData.push(row)}}
$tableDiv=$(".dueDatesTable"),
this.createTable(tableData,true,false,$tableDiv),$tableDiv.find("td").each((i,e)=>{
var text=$(e).text();if(this.isValidDate(text)){
if(text<=luxon.DateTime.now().toISODate())$(e).addClass("isDue")
;$(e).text(luxon.DateTime.fromISO(text).toLocaleString({weekday:"short",
month:"short",day:"2-digit"}))}})},
...
;this.resize()},makeArrayFromStr:function(textStr){
var results=Papa.parse(textStr,{skipEmptyLines:true});return results.data},
preview:function(){
...
if(this.damNums.length>0)var lastDam=this.damNums[this.damNums.length-1],damNum=lastDam+1;else var damNum=1
;this.addDam(damNum);var damSearch=this.damSearch(damNum)
;for(header of headers){var damGen,damGenDOB,damGenStrain
...
var obj={damGen:damGen,damGenDOB:damGenDOB,damGenStrain:damGenStrain}
;this.addDamGenFromTable(obj)}},addSireFromTable:function(obj,headers){
if(this.sireNums.length>0)var lastSire=this.sireNums[this.sireNums.length-1],sireNum=lastSire+1;else var sireNum=1
;this.addSire(sireNum);var sireSearch=this.sireSearch(sireNum)
;for(header of headers){var sireGen,sireGenDOB,sireGenStrain
...
;this.addSireGenFromTable(obj)}this.updateSireList(sireNum)},
upload:function(){var tableStr=this.addedTable;if(tableStr){
var byHeader=Papa.parse(tableStr,{skipEmptyLines:true,header:true
...
;return{matchingObj:matchingObj,noMatches:noMatches}},
updateBreedingWatchDates:function(damNum,damBreedingNum){
var damInfo=this.dams[damNum],breedingInfo=damInfo.breedings[damBreedingNum],plugCheckNums=breedingInfo.plugCheckNums,breedDate=breedingInfo.breedDate,earliestPotentialPlug,earliestLikelyPlug,earliestGoodPlug,sepBreeding,sepPotentialPlug,sepLikelyPlug,sepGoodPlug,birthBreeding,birthPotentialPlug,birthLikelyPlug,birthGoodPlug
;if(breedDate){for(plugCheckNum of plugCheckNums){
var plugCheckInfo=breedingInfo.plugChecks[plugCheckNum],plugState=plugCheckInfo.plugCheck,plugDate=plugCheckInfo.plugDate
;if(plugState>0){
if(!earliestPotentialPlug||plugDate<earliestPotentialPlug)earliestPotentialPlug=plugDate
;if(plugState>1){
if(!earliestLikelyPlug||plugDate<earliestLikelyPlug)earliestLikelyPlug=plugDate
...
var damNums=this.damNums;const damsToCheck=[],damsForLikely=[]
;for(damNum of damNums){
var damInfo=this.dams[damNum],damBreedingNum=this.getDamLatestBreeding(damInfo,dueDate)
;if(damBreedingNum){var breedingInfo=damInfo.breedings[damBreedingNum]
;if((!breedingInfo.sepFromMaleDate||breedingInfo.sepFromMaleDate>dueDate)&&(!breedingInfo.litterDOB||breedingInfo.litterDOB>dueDate)&&(!breedingInfo.stopTrackingDate||breedingInfo.stopTrackingDate>dueDate)&&(!breedingInfo.potentialPlugDate||breedingInfo.potentialPlugDate>dueDate))if(damsToCheck.push(damNum),
!breedingInfo.likelyPlugDate||breedingInfo.likelyPlugDate>dueDate)damsForLikely.push(damNum)
}}return{remaining:damsToCheck,forLikely:damsForLikely}},
getDamsForSeparation:function(dueDate=luxon.DateTime.now().toISODate()){
var damNums=this.damNums;const damsToCheck=[],damsDueToCheck=[]
;for(damNum of damNums){
var damInfo=this.dams[damNum],damBreedingNum=this.getDamLatestBreeding(damInfo,dueDate)
;if(damBreedingNum){var breedingInfo=damInfo.breedings[damBreedingNum]
;if((!breedingInfo.sepFromMaleDate||breedingInfo.sepFromMaleDate>dueDate)&&(!breedingInfo.litterDOB||breedingInfo.litterDOB>dueDate)&&(!breedingInfo.stopTrackingDate||breedingInfo.stopTrackingDate>dueDate))if(damsToCheck.push({
damNum:damNum,damBreedingNum:damBreedingNum
}),breedingInfo.sepBreedingDate<=dueDate)damsDueToCheck.push(damNum)}}
return{remaining:damsToCheck,due:damsDueToCheck}},
getDamsForBirth:function(dueDate=luxon.DateTime.now().toISODate()){
var damNums=this.damNums;const damsToCheck=[],damsDueToCheck=[]
;for(damNum of damNums){
var damInfo=this.dams[damNum],damBreedingNum=this.getDamLatestBreeding(damInfo,dueDate)
;if(damBreedingNum){var breedingInfo=damInfo.breedings[damBreedingNum]
;if((!breedingInfo.litterDOB||breedingInfo.litterDOB>dueDate)&&(!breedingInfo.stopTrackingDate||breedingInfo.stopTrackingDate>dueDate))if(damsToCheck.push({
damNum:damNum,damBreedingNum:damBreedingNum
}),breedingInfo.birthBreedingDate<=dueDate)damsDueToCheck.push(damNum)}}
return{remaining:damsToCheck,due:damsDueToCheck}},
getDamsForImpDates:function(dueDate=luxon.DateTime.now().toISODate()){
var damNums=this.damNums;const damsForTable=[];for(damNum of damNums){
var damInfo=this.dams[damNum],damBreedingNum=this.getDamLatestBreeding(damInfo)
;if(damBreedingNum){var breedingInfo=damInfo.breedings[damBreedingNum]
;if(!breedingInfo.stopTrackingDate||breedingInfo.stopTrackingDate>dueDate)damsForTable.push({
damNum:damNum,damBreedingNum:damBreedingNum})}}return damsForTable},
getDamLatestBreeding:function(damInfo,dueDate=luxon.DateTime.now().toISODate()){
var damBreedingNums=damInfo.breedingNums,latestBreeding,latestBreedingNum
;for(damBreedingNum of damBreedingNums){
var breedingInfo=damInfo.breedings[damBreedingNum],breedDate=breedingInfo.breedDate
;if((!latestBreeding||breedDate>latestBreeding)&&breedDate<dueDate)latestBreeding=breedDate,
latestBreedingNum=damBreedingNum}return latestBreedingNum}};