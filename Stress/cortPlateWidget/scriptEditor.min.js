...
var $divForCopy=$("#forCopy"),listName=$(e.currentTarget).data("list"),numSamples=$(e.currentTarget).data("samples")
;if(!numSamples)if(numSamples=$("#samplesPerMouse").val(),
!numSamples)numSamples=2
;this.copyMouseIDs(listName,$divForCopy,numSamples)
}),$("#idEntry2").each((i,e)=>{}).on("change",e=>{
this.fillSampleIDsFromList($(e.currentTarget))
}),$(".calcTotal").on("change",e=>{
this.calcNumAssignedWells()
}),$("#9ptCurve").on("click",()=>{
$("#numSTD").val(9),$("#largestSTD").val(500),
this.checkAndUpdateWells()
}),$("#7ptCurve").on("click",()=>{
$("#numSTD").val(7),$("#largestSTD").val(250),
this.checkAndUpdateWells()
...
sampleTimes:[],makePlates:function(){
...
;if(this.calcNumAssignedWells())this.updateEntryWells(numNSB,numSTD,numBuffer,numQC,numSamples);else $("#numWarning").text("Did not update; too many samples")
...
;this.checkAndUpdateWells()
;for(var firstSample=this.firstSampleGroup,wellGroup,wellGroupSearch,mouseID,lastMouseID,lastWellGroupSearch,lastWellIDs,isLastDup=false,numDups=1,i=0;i<listLength;i++){
...
if(!isLastDup)$(".plateID"+lastWellGroupSearch).val(lastMouseID+" pre"),
$(lastWellGroupSearch+this.calcSearch("plateID")).text(lastMouseID+" pre"),
this.fillWells("plateID",lastWellIDs,lastMouseID+" pre")
;time=NaN,isLastDup=true,numDups++,thisID=mouseID
}else{
if(isLastDup=false,2==numDups)$(".plateID"+lastWellGroupSearch).val(lastMouseID+" post"),
...
this.fillWells("time",lastWellIDs,5);numDups=1}
...
if(2==numDups)$(".plateID"+lastWellGroupSearch).val(lastMouseID+" post"),$(lastWellGroupSearch+this.calcSearch("plateID")).text(lastMouseID+" post"),
this.fillWells("plateID",lastWellIDs,lastMouseID+" post"),
$(".time"+lastWellGroupSearch).val(5),
this.fillWells("time",lastWellIDs,5);this.resize()
