...
import { shimScript } from './legacy/shim';
import { scriptEditorCode } from './legacy/scriptEditorRaw';

export type MouseEntry = {
  id?: string;
  ...
  numDays?: number;
  dates?: string[];
  [key: string]: any; // allow day_1, day_2, etc.
};
...
type Options = {
  ...
  loadInstances: () => Promise<WidgetInstance[]>;
  saveInstances: (instances: WidgetInstance[]) => Promise<void>;
};

export async function initEstrousWidget(opts: Options) {
  ...
  setTimeout(async () => {
    try {
      // @ts-ignore
      const widget = (window as any).my_widget_script;
      if (!widget) {
      ...
      // @ts-ignore
      widget.parent_class = (window as any).parent_class || {};
  ...
  async function renderInstancesList() {
    ...
    instances.forEach((inst, idx) => {
      ...
      const edit = document.createElement('button'); edit.textContent = 'Open';
  ...
  function formatDateInput(val: string) { return val || ''; }

  async function renderActiveInstance() {
    // Clear and show instance editor
    main.innerHTML = '';
    if (!instances[activeIndex]) return;
    ...
    const miceTitle = document.createElement('h5'); miceTitle.textContent = 'Mice:'; miceDiv.appendChild(miceTitle);
    ...
    const renderMouseList = () => {
      ...
      (inst.mouseNums || []).forEach((mNum) => {
        ...
        const idIn = document.createElement('input'); idIn.value = inst.mice?.[mNum]?.id || '';
        idIn.placeholder = 'ID';
        idIn.oninput = () => { inst.mice[mNum].id = idIn.value; opts.saveInstances(instances); };
        row.appendChild(idIn);
      const cycleIn = document.createElement('input'); cycleIn.value = String(inst.mice?.[mNum]?.cycleNum ?? '');
        cycleIn.placeholder = 'Cycle #'; cycleIn.oninput = () => { inst.mice[mNum].cycleNum = cycleIn.value; opts.saveInstances(instances); };
        row.appendChild(cycleIn);
        const startIn = document.createElement('input'); startIn.type='date'; startIn.value = inst.mice?.[mNum]?.startDate || '';
        ...
        const endIn = document.createElement('input'); endIn.type='date'; endIn.value = inst.mice?.[mNum]?.endDate || '';
        ...
        const delBtn = document.createElement('button'); delBtn.textContent='Delete'; delBtn.onclick = async ()=>{ delete inst.mice[mNum]; inst.mouseNums = inst.mouseNums.filter(x=>x!==mNum); await opts.saveInstances(instances); renderActiveInstance(); };
    ...
    const addMouseBtn = document.createElement('button'); addMouseBtn.textContent = 'Add mouse';
    addMouseBtn.onclick = async () => { let next = 1; if (inst.mouseNums && inst.mouseNums.length) next = Math.max(...inst.mouseNums)+1; inst.mouseNums = inst.mouseNums || []; inst.mouseNums.push(next); inst.mice[next] = { id: '', cycleNum: '', startDate: '', endDate: '' }; await opts.saveInstances(instances); renderActiveInstance(); };
    ...
    const cardContainer = document.createElement('div'); cardContainer.style.marginTop='12px'; main.appendChild(cardContainer);
      ...
      inst.mouseNums.forEach((m) => {
        ...
        if (mObj.startDate && mObj.endDate) {
          ...
          for (let d = new Date(s); d <= e; d.setDate(d.getDate()+1)) {
            ...
            if (!inst.dates![iso][m]) inst.dates![iso][m] = 1; // placeholder day numbering not strictly accurate here
          }
        }
      });
    }

    function adjustScoringRowsForMouse(mNum: number) {
      // rebuild dates index and ensure scores exist (we'll store scores under inst.mice[mNum].scores)
      rebuildDatesIndex();
      if (!inst.mice[mNum].dates) inst.mice[mNum].dates = [];
      ...
      for (const date of Object.keys(inst.dates || {})) {
        if (inst.dates && inst.dates[date] && inst.dates[date].hasOwnProperty(mNum)) {
          if (!inst.mice) inst.mice = {} as any;
          if (!inst.mice[mNum]) inst.mice[mNum] = { id: '', cycleNum: '', startDate: '', endDate: '', dates: [] } as any;
          if (!inst.mice[mNum].dates) inst.mice[mNum].dates = [];
          (inst.mice[mNum].dates as any).push(date);
        }
      }
    }

    // Render basic table
    const tableDiv = document.createElement('div'); tableDiv.style.marginTop='12px'; main.appendChild(tableDiv);
    const table = document.createElement('table'); table.border = '1'; table.style.borderCollapse='collapse'; tableDiv.appendChild(table);

    function renderTable() {
      table.innerHTML = '';
      const thead = document.createElement('thead');
      const headerRow = document.createElement('tr'); ['mouseID','cycleID','startDate'].forEach(h=>{const th=document.createElement('th'); th.textContent=h; headerRow.appendChild(th);});
      // compute days max
  const maxDays = Math.max(...(inst.mouseNums.map(m => (inst.mice && inst.mice[m] && inst.mice[m].dates) ? (inst.mice[m].dates as any).length : 0)),0);
      for (let i=1;i<=maxDays;i++){const th=document.createElement('th'); th.textContent='Day'+i; headerRow.appendChild(th);} thead.appendChild(headerRow); table.appendChild(thead);
      const tbody = document.createElement('tbody');
      inst.mouseNums.forEach((m)=>{
        const tr = document.createElement('tr');
        const tdId = document.createElement('td'); tdId.textContent = inst.mice[m].id || `Mouse ${m}`; tr.appendChild(tdId);
        const tdCycle = document.createElement('td'); tdCycle.textContent = String(inst.mice[m].cycleNum||''); tr.appendChild(tdCycle);
        const tdStart = document.createElement('td'); tdStart.textContent = inst.mice[m].startDate||''; tr.appendChild(tdStart);
        const days = inst.mice[m].dates||[];
        for (let i=0;i<maxDays;i++){
          const td = document.createElement('td');
          const dayDate = days[i];
          const input = document.createElement('input'); input.style.width='60px';
    ...
    const chartBtn = document.createElement('button'); chartBtn.textContent='Update Charts'; chartBtn.onclick = ()=>{ makeCharts(); }; main.appendChild(chartBtn);
    const chartDiv = document.createElement('div'); chartDiv.id='chartDiv'; chartDiv.style.marginTop='8px'; main.appendChild(chartDiv);
    ...
    async function makeCharts(){
      ...
      for (const m of inst.mouseNums) {
        ...
        const div = document.createElement('div'); div.style.height='200px'; div.id = 'chart_'+m; chartDiv.appendChild(div);
    ...
    async function loadGoogleCharts(){
      return new Promise<void>((resolve)=>{
        const script = document.createElement('script'); script.src='https://www.gstatic.com/charts/loader.js'; script.onload = ()=>{(window as any).google.charts.load('current',{packages:['corechart','line'], callback: ()=> resolve() });}; document.head.appendChild(script);
      });
    }

    function drawGoogleChart(rowArray: any[], mouseID: string, mouseNum: number, elementId: string){
      const google = (window as any).google;
      if(!google || !google.visualization) return;
      const data = new google.visualization.DataTable();
      data.addColumn('number','Day'); data.addColumn('number','Stage');
  ...
  addInstanceBtn.onclick = async ()=>{
    const inst: WidgetInstance = { name: 'New instance', mouseNums: [], mice: {}, dates: {} };
    instances.push(inst);
